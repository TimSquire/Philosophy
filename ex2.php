...
if(ISSET($_POST['submitButton'])){
	...
	if(!empty($fname) && !empty($sname) && !empty($user) && !empty($pwd) && !empty($email) && !empty($dob)){
		...
		if(empty($msg)){
			...
			$query = "INSERT INTO `users` (`user_id`, `fname`, `sname`, `username`, `password`, `dob`, `email`) VALUES (NULL, '$fname', '$sname', '$user', '$hashed', '$dob', '$email')";
			$query2 = "SELECT * FROM `users` WHERE `username` = '$user'";
			$result = mysqli_query($conn, $query2) or die('No connecto');
